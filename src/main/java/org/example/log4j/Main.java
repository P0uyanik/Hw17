...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        FileHandler fileHandler = new FileHandler("pouya.log", true);
        logger.addHandler(fileHandler);
        logger.setLevel(Level.ALL);


        PersonService<Person> personService = new PersonServiceImpl<>();
        System.out.println(personService.singUp("Pouya", "Nik"));

        StudentServiceImpl studentService = new StudentServiceImpl();
        System.out.println(studentService.signUpStudent("std", "ali",
                883L,"asdcc",
                "1992"));
